...
from ...models.sutrack.compression.ce import generate_mask_cond
    ...
    def track(self, image, info: dict = None):
        ...
        if self.debug == 1:
            ...
            if ('removed_indexes_x' in aux_dict and aux_dict['removed_indexes_x'] is not None and
                ...
                # evit可视化
                if ('extra_indexes_x' in aux_dict and aux_dict['extra_indexes_x'] is not None):
                    extra_indexes_x = aux_dict.get('extra_indexes_x', None)
                    if x_dte_patch_arr_show is not None:
                        masked_search = gen_visualization(image=x_rgb_patch_arr_show,
                                                          ...
                                                          merged_indices=removed_indexes_x,
                                                          host_indices=extra_indexes_x,
                                                          extra_indices=extra_indexes_x,
                                                          dte=True,
                                                          image_dte=x_dte_patch_arr_show)
                    else:
                        masked_search = gen_visualization(image=x_patch_arr,
                                                          mask_indices=removed_indexes_x,
                                                          merged_indices=removed_indexes_x,
                                                          host_indices=extra_indexes_x,
                                                          extra_indices=extra_indexes_x, )
