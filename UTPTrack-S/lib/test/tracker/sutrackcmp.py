...
from lib.models.sutrackcmp import build_sutrackcmp
...
from ...models.sutrackcmp.compression.ce import generate_mask_cond

class SUTRACKCMP(BaseTracker):
    def __init__(self, params, dataset_name):
        super(SUTRACKCMP, self).__init__(params)
        network = build_sutrackcmp(params.cfg)
        ...
        task_index_batch = [self.cfg.MODEL.TASK_INDEX[DATASET_NAME]]
        task_index_batch = torch.tensor(task_index_batch).cuda()  # torch.Size([bs])
        self.task_index_batch = task_index_batch

        self.feat_sz = self.cfg.TEST.SEARCH_SIZE // self.cfg.MODEL.ENCODER.STRIDE   # 临时增加的，为了可视化
        self.template_sz = self.cfg.TEST.TEMPLATE_SIZE // self.cfg.MODEL.ENCODER.STRIDE   # 临时增加的，为了可视化
    ...
    def initialize(self, image, info: dict):
        ...
        if self.cfg.MODEL.ENCODER.CE_LOC:
            self.box_mask_z = generate_mask_cond(self.cfg, 1, template.device)
    ...
    def track(self, image, info: dict = None):
        ...
        # update the text_src
        if self.use_nlp and info is not None and "init_nlp" in info:
            # print(self.frame_id)
            if len(set(info.get("init_nlp"))) == 1:
                pass
            else:
                init_nlp = info.get("init_nlp")[self.frame_id]
                text_data, _ = self.extract_token_from_nlp_clip(init_nlp)
        ...
        if image.shape[-1] == 6:
            ...
            if self.num_template == 1:
                ...
                for i in range(self.num_template):
                    ...
                    z_rgb_patch_arr_show = temp_z_patch_arr[:, :, :3]
                    z_dte_patch_arr_show = temp_z_patch_arr[:, :, 3:]
                    z_patch_arr_list_show.append(z_rgb_patch_arr_show)
                    z_patch_arr_list_show.append(z_dte_patch_arr_show)
                zd_rgb_patch_arr_show =  self.z_patch_arr_list[-1][:, :, :3]
                zd_dte_patch_arr_show = self.z_patch_arr_list[-1][:, :, 3:]
        else:
            image_show = image
            ...
            if self.num_template == 1:
                ...
                zd_rgb_patch_arr_show = z_patch_arr_list_show[-1]
        ...
        if self.debug == 1:
            ...
            if 'removed_indexes_x' in aux_dict and aux_dict['removed_indexes_x'] is not None:
                ...
                # # Merge info
                # merged_indexes_x = aux_dict.get('merged_indexes_x', None)
                # host_indexes_x = aux_dict.get('host_indexes_x', None)
                #
                # if merged_indexes_x is not None:
                #     merged_indexes_x = [merged_indexes_x_i.cpu().numpy() for merged_indexes_x_i in merged_indexes_x]
                # if host_indexes_x is not None:
                #     host_indexes_x = [host_indexes_x_i.cpu().numpy() for host_indexes_x_i in host_indexes_x]

                if x_dte_patch_arr_show is not None:
                    masked_search = gen_visualization(image=x_rgb_patch_arr_show, mask_indices=removed_indexes_x, dte=True, image_dte=x_dte_patch_arr_show)
                else:
                    masked_search = gen_visualization(image=x_patch_arr, mask_indices=removed_indexes_x)
                self.visdom.register(torch.from_numpy(masked_search).permute(2, 0, 1), 'image', 1, 'masked_search')

            # 可视化动态模板的剪枝效果
            if 'removed_indexes_dz' in aux_dict and aux_dict['removed_indexes_dz'] is not None:
                assert self.num_template == 2, "TE only support 2 template (1 static template + 1 dynamic template)"
                removed_indexes_dz = aux_dict['removed_indexes_dz']
                removed_indexes_dz = [(removed_indexes_dz_i.cpu().numpy()-self.template_sz**2) for removed_indexes_dz_i in removed_indexes_dz]

                if zd_dte_patch_arr_show is not None:
                    masked_dynamic_template = gen_visualization(image=zd_rgb_patch_arr_show, mask_indices=removed_indexes_dz, dte=True, image_dte=zd_dte_patch_arr_show)
                else:
                    masked_dynamic_template = gen_visualization(zd_rgb_patch_arr_show, mask_indices=removed_indexes_dz)
                self.visdom.register(torch.from_numpy(masked_dynamic_template).permute(2, 0, 1), 'image', 1, 'masked_dynamic_template')

            # 可视化静态模板的剪枝效果
            if 'removed_indexes_sz' in aux_dict and aux_dict['removed_indexes_sz'] is not None:
                removed_indexes_sz = aux_dict['removed_indexes_sz']
                removed_indexes_sz = [(removed_indexes_sz_i.cpu().numpy()-self.template_sz**2) for removed_indexes_sz_i in removed_indexes_sz]

                if zd_dte_patch_arr_show is not None:
                    masked_static_template = gen_visualization(image=zd_rgb_patch_arr_show, mask_indices=removed_indexes_sz, dte=True, image_dte=zd_dte_patch_arr_show)
                else:
                    masked_static_template = gen_visualization(zd_rgb_patch_arr_show, mask_indices=removed_indexes_sz)
                self.visdom.register(torch.from_numpy(masked_static_template).permute(2, 0, 1), 'image', 1, 'masked_static_template')
