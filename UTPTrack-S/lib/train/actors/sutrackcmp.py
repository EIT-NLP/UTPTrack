...
from ...models.sutrackcmp.compression.ce import generate_mask_cond, adjust_keep_rate

class SUTrackcmpActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        for key in data['dataset']:
            ...
            if 'LASOT' in key:
                key = 'LASOT'
            if 'TRACKINGNET' in key:
                key = 'TRACKINGNET'
            task_index_batch.append(self.cfg.MODEL.TASK_INDEX[key])
        ...
        dte_keep_rate = None
        if self.cfg.MODEL.ENCODER.DTE_LOC:
            ce_start_epoch = self.cfg.TRAIN.CE_START_EPOCH
            ce_warm_epoch = self.cfg.TRAIN.CE_WARM_EPOCH
            dte_keep_rate = adjust_keep_rate(data['epoch'],
                                            warmup_epochs=ce_start_epoch,
                                            total_epochs=ce_start_epoch + ce_warm_epoch,
                                            ITERS_PER_EPOCH=1,
                                            base_keep_rate=self.cfg.MODEL.ENCODER.DTE_KEEP_RATIO[0])
        ste_keep_rate = None
        if self.cfg.MODEL.ENCODER.STE_LOC:
            ste_keep_rate = adjust_keep_rate(data['epoch'],
                                             warmup_epochs=ce_start_epoch,
                                             total_epochs=ce_start_epoch + ce_warm_epoch,
                                             ITERS_PER_EPOCH=1,
                                             base_keep_rate=self.cfg.MODEL.ENCODER.STE_KEEP_RATIO[0])
        # print("******************** CE & TE ********************")
        # print(f"Epoch: {data['epoch']}")
        # print(f"-----> ce keep rate: {ce_keep_rate}")
        # print(f"-----> dte keep rate: {dte_keep_rate}")
        # print(f"-----> ste keep rate: {ste_keep_rate}")
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        # weighted sum
        loss = self.loss_weight['giou'] * giou_loss + self.loss_weight['l1'] * l1_loss + self.loss_weight['focal'] * location_loss + self.loss_weight['task_cls'] * task_cls_loss
