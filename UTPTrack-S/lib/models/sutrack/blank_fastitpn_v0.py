"""
    这个代码是不包含任何压缩方法的空白模板，是完整的，另一个blank——fastitpn是精简过的，删除了一些用不到的分支
"""
...
class BlankAttention(Attention):
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        ...
        if self.relative_position_bias_table is not None:   # 默认没有用这个
        ...
        if self.rel_pos_bias is not None:   # 默认没有用这个
            attn = attn + self.rel_pos_bias().type_as(attn)

        if rel_pos_bias is not None:    # 默认没有用这个
...
class Blank_Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        if use_shared_rel_pos_bias: # 默认是没有用这个的
            self.rel_pos_bias = RelativePositionBias(window_size=self.patch_embed.patch_shape, num_heads=num_heads)
        else:
            self.rel_pos_bias = None

        if use_shared_decoupled_rel_pos_bias:   # 默认是没有用这个的
