...
class CE_HiViTAttentionBlock(nn.Module):
    ...
    def forward(self, xz, rel_pos_bias=None, attn_mask=None, global_index_z=None, global_index_x=None, ce_template_mask=None, keep_ratio_x=None, num_template=1):
        ...
        if self.keep_ratio_x < 1 and (keep_ratio_x is None or keep_ratio_x < 1):
            ...
            xz, global_index_x, removed_index_x = candidate_elimination(
                xz, attn, lens_z, keep_ratio_x, global_index_x, ce_template_mask, num_template
            )

        xz = xz + self.drop_path(self.gamma_2 * self.mlp(self.norm2(xz)))
        return xz, global_index_z, global_index_x, removed_index_x, attn

class CE_Fast_iTPN(nn.Module):
    ...
    def forward_features(self, template_list, search_list, template_anno_list, text_src, task_index,
                         ce_template_mask=None, ce_keep_rate=None, return_last_attn=True):
        ...
        for i, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            if self.grad_ckpt:
                xz, global_index_z, global_index_x, removed_index_x, attn = checkpoint.checkpoint(
                    blk, xz, rel_pos_bias, global_index_z=global_index_z, global_index_x=global_index_x,
                    ce_template_mask=ce_template_mask, keep_ratio_x=ce_keep_rate, num_template=self.num_template,
                )
            else:
            ...
            if self.ce_loc is not None and i in self.ce_loc:
                ...
                # print(f"Removed Index: {removed_index_x}")
    ...
    def forward(self, template_list, search_list, template_anno_list, text_src, task_index,
        ...
        xz, aux_dict = self.forward_features(template_list,
                                             search_list,
                                             template_anno_list,
                                             text_src,
                                             task_index,
...
@register_model
def ce_fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CE_Fast_iTPN(
...
@register_model
def ce_fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CE_Fast_iTPN(
...
@register_model
def ce_fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CE_Fast_iTPN(
...
@register_model
def ce_fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CE_Fast_iTPN(
