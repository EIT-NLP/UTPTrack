...
from .compression.tome import parse_r, merge_wavg_bsm


class ToMeAttention(Attention):
    def forward(self, x, rel_pos_bias=None, attn_mask=None, size=None):
        B, N, C = x.shape
        ...
        if self.deepnorm or self.subln:
            ...
            k = k.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)  # B, num_heads, N, C
            v = v.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)  # B, num_heads, N, C
        else:
            qkv_bias = None
...
class ToMe_HiViTAttentionBlock(nn.Module):
    ...
    def forward(self, xz, rel_pos_bias=None, attn_mask=None, global_index_z=None, global_index_x=None):
        ...
        xz_attn, metric = self.attn(self.norm1(xz), rel_pos_bias=rel_pos_bias, attn_mask=attn_mask, size=attn_size)
        xz = xz + self.drop_path(self.gamma_1 * xz_attn) # (10,866,512)
        ...
        if r > 0:
            ...
            xz, self._tome_info["size"], global_index_x, removed_index_x = merge_wavg_bsm(
                xz, metric, global_index_x, r, lens_z, self._tome_info["class_token"], self._tome_info["text_token"], self._tome_info["size"]
            )

        xz = xz + self.drop_path(self.gamma_2 * self.mlp(self.norm2(xz)))
        return xz, global_index_z, global_index_x, removed_index_x


class ToMe_Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        for blk in self.blocks:
            if isinstance(blk, ToMe_HiViTAttentionBlock):
    ...
    def forward_features(self, template_list, search_list, template_anno_list, text_src, task_index,
        ...
        for blk_idx, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            # print(f"block idx: {blk_idx}")
            # print(f"**********global_index_x: {global_index_x.shape}*************")
            # print(global_index_x)
            # print("**********removed_indexes_x*************")
            # if removed_indexes_x != []:
            #     print(removed_indexes_x[-1])
            if self.grad_ckpt:
                xz, global_index_z, global_index_x, removed_index_x = checkpoint.checkpoint(
                    blk, xz, rel_pos_bias, global_index_z=global_index_z, global_index_x=global_index_x)
            else:
                xz, global_index_z, global_index_x, removed_index_x = blk(
                    xz, rel_pos_bias, global_index_z=global_index_z, global_index_x=global_index_x)

            if self.r > 0:
                removed_indexes_x.append(removed_index_x)
            # print(f"blk_idx: {blk_idx}, xz: {xz.shape}")
            token_count_per_block.append(xz.shape[1])

        xz = self.norm(xz)  # (b, n, c) [cls_tokens, x, z, text_src]
    ...
    def forward(self, template_list, search_list, template_anno_list, text_src, task_index, ce_template_mask=None, ce_keep_rate=None):
...
@register_model
def tome_fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = ToMe_Fast_iTPN(
...
@register_model
def tome_fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = ToMe_Fast_iTPN(
...
@register_model
def tome_fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = ToMe_Fast_iTPN(
...
@register_model
def tome_fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = ToMe_Fast_iTPN(
