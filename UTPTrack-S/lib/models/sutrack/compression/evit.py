...
def evit_cem(tokens: torch.Tensor, attn: torch.Tensor, lens_z: int, keep_ratio: float,
    ...
    attn_z = attn_z.mean(dim=2).mean(dim=1)  # B, L_x
    ...
    non_topk_attn, non_topk_idx = sorted_attn[:, lens_keep+1:], indices[:, lens_keep+1:]
    ...
    non_topk_tokens = tokens_x.gather(dim=1, index=non_topk_idx.unsqueeze(-1).expand(B, -1, C))
    extra_token = torch.sum(non_topk_tokens * non_topk_attn.unsqueeze(-1), dim=1, keepdim=True)
    ...
    # concatenate new token sequence: cls + topk x + extra + z + text
