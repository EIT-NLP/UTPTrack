...
# 二分图软匹配算法,加权平均
def merge_wavg_bsm(xz: torch.Tensor, metric: torch.Tensor, global_index_x: torch.Tensor, r: int, lens_z: int,
                   class_token: bool = False, text_token: bool = False, size: torch.Tensor = None, mode: str = "wavg"):

    lens_x = metric.shape[1] - 2 - lens_z
    protected = lens_z
    if class_token:
        protected += 1
    if text_token:
    ...
    text_size = None
    if text_token:
        text_part = xz[:, start_idx:start_idx + 1, :]
        text_size = size[:, start_idx:start_idx + 1, :]
    ...
    with torch.no_grad():
        ...
        edge_idx = node_max.argsort(dim=-1, descending=True)[..., None] # # 按相似度排序
    ...
    src_bsm_idx = src_idx.squeeze(-1) * 2  # 当前序列的偶数位置，偶数组会被合并的token的原始idx
    ...
    def apply_weighted_merge(x_tensor, size_tensor):
        ...
        if mode == "wavg":  # 加权平均
            ...
            # 累加权重到目标位置
            dst_size = dst_size.scatter_add(-2, dst_idx.expand(n, r, 1), src_to_merge_size)
            # 计算加权平均：避免除零
        ...
        merged_x = x_keep_tokens.gather(dim=1, index=sorted_keep_bsm_idx.unsqueeze(-1).expand(-1, -1, c))
        merged_x_size = x_keep_sizes.gather(dim=1, index=sorted_keep_bsm_idx.unsqueeze(-1).expand(-1, -1, 1))
        merged_x_idx = x_keep_bsml_idx.gather(dim=1, index=sorted_keep_bsm_idx)  # [B, M]
    ...
    if text_token:
        ...
        combined_sizes.append(text_size)
