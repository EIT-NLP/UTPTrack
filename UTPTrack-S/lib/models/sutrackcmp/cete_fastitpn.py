...
from .compression.ce import candidate_elimination
from .compression.te import template_elimination
...
class CETE_HiViTAttentionBlock(nn.Module):
    ...
    def forward(self, xz, rel_pos_bias=None, attn_mask=None, global_index_z=None, global_index_x=None,
        ...
        if self.keep_ratio_dz < 1 and (keep_ratio_dz is None or keep_ratio_dz < 1):    # 这里已经控制在TE_LOC才进入，验证过
            keep_ratio_dz = self.keep_ratio_dz if keep_ratio_dz is None else keep_ratio_dz
            # print(f"-----> DTE: {keep_ratio_dz}")
...
class CETE_Fast_iTPN(nn.Module):
    ...
    def forward_features(self, template_list, search_list, template_anno_list, text_src, task_index,
        ...
        for i, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            # if i in self.ce_loc or i in self.te_loc:
            #     print(f"Attention blk {i}: {ce_keep_rate} {dte_keep_rate}")
            if self.grad_ckpt:
                xz, global_index_z, removed_index_z, global_index_x, removed_index_x, attn = checkpoint.checkpoint(
                    blk, xz, rel_pos_bias,
            ...
            if self.dte_loc is not None and i in self.dte_loc:
                ...
                # print(f"Removed Z's Index: {removed_index_z}")
        ...
        aux_dict = {
            ...
            "removed_indexes_dz": removed_indexes_z, # used for visualization
...
@register_model
def cete_fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CETE_Fast_iTPN(
...
@register_model
def cete_fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CETE_Fast_iTPN(
...
@register_model
def cete_fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CETE_Fast_iTPN(
...
@register_model
def cete_fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CETE_Fast_iTPN(
