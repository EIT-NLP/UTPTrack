...
from .compression.ce import candidate_elimination_by_ate_mma
from .compression.ate import dynamic_template_elimination_mma, static_template_elimination_mma
...
class CEATETTAMMA_HiViTAttentionBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 multimodal_aware_pruning_target=None,
                 ):
        super().__init__()
        ...
        self.multimodal_aware_pruning_target = multimodal_aware_pruning_target ###
    ...
    def forward(self, xz, rel_pos_bias=None, attn_mask=None,
        ...
        if self.keep_ratio_x < 1 and (keep_ratio_x is None or keep_ratio_x < 1):    # 这里已经控制在CE_LOC才进入，验证过
            ...
            xz, global_index_x, removed_index_x = candidate_elimination_by_ate_mma(
                ...
                mmaware_pruning_target = self.multimodal_aware_pruning_target,
            )
        ...
        if self.keep_ratio_dz < 1 and (keep_ratio_dz is None or keep_ratio_dz < 1):
            ...
            xz, global_index_dz, removed_index_dz = dynamic_template_elimination_mma(
        ...
        if self.keep_ratio_sz < 1 and (keep_ratio_sz is None or keep_ratio_sz < 1):
            ...
            xz, global_index_sz, removed_index_sz, ce_template_mask, z_indicate_mask = static_template_elimination_mma(
...
class CEATETTAMMA_Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 ste_keep_ratio=None,  ###
                 token_type_aware_pruing=None,      ###
                 multimodal_aware_pruning=False,    ###
                 multimodal_aware_pruning_target=None,  ###
                 **kwargs):
        super().__init__()
        ...
        self.token_type_aware_pruing = token_type_aware_pruing
        self.multimodal_aware_pruning = multimodal_aware_pruning
        self.multimodal_aware_pruning_target = multimodal_aware_pruning_target
    ...
    def build_blocks(self,
        ...
        for i in range(depths[2]):
            ...
            if ste_loc is not None and i in ste_loc:
                ...
                ste_index += 1
            self.blocks.extend([
                CEATETTAMMA_HiViTAttentionBlock(
                    dim=dims['16'],
                    num_heads=num_heads,
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    ...
                    multimodal_aware_pruning_target=self.multimodal_aware_pruning_target,
                )
            ])
    ...
    def forward_features(self, template_list, search_list, template_anno_list, text_src, task_index,
        ...
        if self.multimodal_aware_pruning is False:
            task_index = None
...
@register_model
def ceatettamma_fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CEATETTAMMA_Fast_iTPN(
...
@register_model
def ceatettamma_fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CEATETTAMMA_Fast_iTPN(
...
@register_model
def ceatettamma_fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CEATETTAMMA_Fast_iTPN(
...
@register_model
def ceatettamma_fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = CEATETTAMMA_Fast_iTPN(
