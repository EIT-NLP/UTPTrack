...
from lib.models.sutrackcmpvit import fastitpn as fastitpn_module
from lib.models.sutrackcmpvit import blank_fastitpn as blank_fastitpn_module
from lib.models.sutrackcmpvit import dyvit_fastitpn as dyvit_fastitpn_module
...
class EncoderBase(nn.Module):
    ...
    def forward(self, template_list, search_list, template_anno_list, text_src, task_index, keep_rate=None):
        xs, aux_dict = self.body(template_list, search_list, template_anno_list, text_src, task_index, keep_rate=keep_rate)
        return xs, aux_dict
...
class Encoder(EncoderBase):
    ...
    def __init__(self, name: str, train_encoder: bool, search_size: int, template_size: int, open_layers: list, cfg=None):
        if "dyvit_fastitpn" in name.lower():
            encoder = getattr(dyvit_fastitpn_module, name)(
                ...
                pruning_loc=cfg.MODEL.ENCODER.PRUNING_LOC,
                keep_ratio=cfg.MODEL.ENCODER.KEEP_RATIO,
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
