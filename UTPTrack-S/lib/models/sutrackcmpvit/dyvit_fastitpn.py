...
class DyViTHiViTAttentionBlock(nn.Module):
    ...
    def forward(self, xz, rel_pos_bias=None, attn_mask=None, global_index_z=None, global_index_x=None,
                keep_ratio=None, num_template=1, score_predictor=None, prev_decision=None, policy=None):
        """
        ...
        x = xz[:, 1:1+lens_x]
        z = xz[:, 1+lens_x:-1]
        text = xz[:, -1].unsqueeze(1)

        removed_index_x = None
        keep_index_x = global_index_x
        if score_predictor is not None and self.keep_ratio < 1 and (keep_ratio is None or keep_ratio < 1):
            ...
            if self.training: # 训练时使用Gumbel Softmax进行可微分采样,没有实际剪枝，此时的policy,表示保留概率
                policy_cls = policy[:, 0].unsqueeze(1)
                # policy_x = policy[:, 1:1 + lens_x]
                policy_z = policy[:, 1 + lens_x:-1]
                policy_text = policy[:, -1].unsqueeze(1)

                hard_keep_decision = F.gumbel_softmax(pred_score, hard=True)[:, :, 0:1] * prev_decision
                policy = torch.cat([policy_cls, hard_keep_decision, policy_z, policy_text], dim=1)
                ...
                _, L, _ = x.shape
                attentive_tokens_x = x.gather(dim=1, index=topk_idx.unsqueeze(-1).expand(B, -1, C))
                xz = torch.cat([cls, attentive_tokens_x, z, text], dim=1)
                prev_decision = prev_decision.gather(dim=1, index=topk_idx.unsqueeze(-1).expand(B, -1, 1))

        xz_attn = self.attn(self.norm1(xz), rel_pos_bias=rel_pos_bias, attn_mask=attn_mask, policy=policy)
        xz = xz + self.drop_path(self.gamma_1 * xz_attn)
        xz = xz + self.drop_path(self.gamma_2 * self.mlp(self.norm2(xz)))
        return xz, global_index_z, keep_index_x, removed_index_x, prev_decision, policy
    
class DyViT_Fast_iTPN(nn.Module):
    ...
    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index,
                         keep_rate=None, return_last_attn=False):
        aux_dict = {
        ...
        if self.training:
            policy = torch.ones(B, 2+lens_x+lens_z, 1, dtype=xz.dtype, device=xz.device)
        else:
            policy = None
        for i, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            ...
            if self.grad_ckpt:
                xz, global_index_z, global_index_x, removed_index_x, prev_decision, policy = checkpoint.checkpoint(
                    blk, xz, rel_pos_bias, global_index_z=global_index_z, global_index_x=global_index_x,
                    keep_ratio=keep_rate, num_template=self.num_template,
                    score_predictor=score_predictor, prev_decision=prev_decision, policy=policy,
                )
            else:
                xz, global_index_z, global_index_x, removed_index_x, prev_decision, policy = blk(
        ...
        aux_dict = {
            # "attn": attn,
            "removed_indexes_x": removed_indexes_x,
            "token_count_per_block": token_count_per_block,
        }

        return xz, aux_dict

    def forward(self, template_list, search_list, template_anno_list, text_src, task_index, keep_rate=None):
...
@register_model
def dyvit_fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = DyViT_Fast_iTPN(
...
@register_model
def dyvit_fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = DyViT_Fast_iTPN(
...
@register_model
def dyvit_fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = DyViT_Fast_iTPN(
...
@register_model
def dyvit_fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = DyViT_Fast_iTPN(
