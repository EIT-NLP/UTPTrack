...
def genConfig(seq_path, set_type):
    if set_type == 'RGBT234':
        ...
        raise NotImplementedError("ERROR: dataset not reproduce!")
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            region, confidence = tracker.track(image, info)  # xywh
...
class VEGETA_RGBT(object):
    ...
    def track(self, img_RGB, info):
        '''TRACK'''
        outputs = self.tracker.track(img_RGB, info)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='sutrackcmp', help='Name of tracking method(ostrack, prompt, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='cemtem_b224', help='Name of tracking method.')
    ...
    if dataset_name == 'RGBT234':
        ...
        seq_home = '/home/wuhao/workspace/datasets/lasher/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home, f))]
        seq_list.sort()
    else:
